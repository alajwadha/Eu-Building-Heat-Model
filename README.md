...
| `scripts/` | every step the reproduction guide names: the dispatch, the merit orders, the capital-recovery and capacity-payment calculations, the sensitivity sweeps and the numbers gate |
...
Individual steps run as modules from this directory, for example:
...
manual download and is committed here. The EUBUCCO v0.2 building footprints behind the
bottom-up demand basis come from the EUBUCCO object store; the forward model runs from
the committed intermediate tables in `data/processed/` without them.
